...
Once one depends on an impure function, the whole concept of purity is violated. 
Yet it is unclear if striving for purity in an impure world is reasonable. 
I suppose it is not, as big advantages of purity such as referential transparency and easier parallelism are lost once an impure function is called. 
Additionally both languages' compilers do not make use of the advantages purity can offer. 
...
Or as one of the language designers Rob Pike said about a map/reduce/filter package he wrote: 
...
Elixir is different in this regards. 
